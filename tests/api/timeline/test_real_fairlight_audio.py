...
class FairlightAudioTester:
    ...
    async def test_fairlight_effect_chain(self) -> bool:
        ...
        try:
            ...
            if len(effect_list) != len(effects_order):
                logger.error(
                    f"✗ Expected {len(effects_order)} effects, got {len(effect_list)}"
                )
                return False

            logger.info(
                f"✓ Fairlight effect chain created with {len(effect_list)} effects"
            )
            return True
    ...
    async def test_fairlight_effect_parameters(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "  Gate parameters: Threshold=-30dB, Attack=0.5ms, Release=100ms"
            )
    ...
    async def test_audio_level_monitoring(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"✓ Audio level monitoring validated ({len(test_levels)} readings)"
            )
            return True
    ...
    async def test_audio_normalization(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"  Before normalization: Level={track.GetLevel()} dB, LUFS={track.GetLUFSLevel()}"
            )
            ...
            logger.info(
                f"  After normalization: Level={track.GetLevel()} dB, LUFS={track.GetLUFSLevel()}"
            )
