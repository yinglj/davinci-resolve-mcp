...
class ResolveAgent:
    ...
    def __init__(self, resolve_server=None):
        ...
        system_instruction = """You are a DaVinci Resolve AI Copilot.
You help users with video editing, color grading, and workflow automation.
When you need to perform an action:
1. Search for appropriate tools using 'search_resolve_tools' if you are unsure which tool to use.
2. If you find a specific tool, use 'execute_resolve_tool' to run it.
3. Always validate the result and suggest next steps.
You have access to 300+ DaVinci Resolve API tools through this dynamic interface."""
    ...
    async def process_request(
        ...
        try:
            ...
            plan = await self.planner.create_plan(
                user_request, self.context, self.doc_rag
            )
            ...
            try:
                ...
                if validation.is_valid:
                    ...
                    fixed_plan = await self.feedback_loop.fix_plan(
                        plan, validation.errors, self.doc_rag
                    )
                    plan = fixed_plan
                    current_retry += 1
    ...
    async def analyze_video(
        self, video_path: str, analysis_type: str = "general"
    ) -> Dict[str, Any]:
        """
    ...
    async def suggest_next_actions(self) -> List[str]:
        ...
        return await self.planner.suggest_next_actions(
            self.context, self.memory.get_recent_actions()
        )
