...
# Configure logging
logger = logging.getLogger("davinci-resolve-mcp.project_operations")
...
# Operations migrated from src/api/project_operations.py
...
def register_tools(proxy) -> int:
    """Register project tools with the given proxy (ToolProxy)."""
    try:
    ...
    # Keep names compatible with src/api/project_operations.py
    if not _already_registered("set_project_setting"):
        proxy.register_tool(
            "set_project_setting",
            lambda setting_name, value: set_project_setting(
                get_resolve() if get_resolve else None, setting_name, value
            ),
            "project",
    ...
    if not _already_registered("set_timeline_format"):
        proxy.register_tool(
            "set_timeline_format",
            lambda width, height, frame_rate: set_timeline_format(
                get_resolve() if get_resolve else None, width, height, frame_rate
            ),
            "project",
            "Set timeline resolution and frame rate",
            {
