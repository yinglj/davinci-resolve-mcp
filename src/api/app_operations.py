...
def quit_resolve_app(
    ...
    try:
        ...
        if sys_platform == "darwin":
            ...
            if force:
                subprocess.run(
                    ["taskkill", "/F", "/IM", "Resolve.exe"], stdin=subprocess.DEVNULL
                )
            else:
                subprocess.run(
                    ["taskkill", "/IM", "Resolve.exe"], stdin=subprocess.DEVNULL
                )
            return True
            ...
            if force:
                subprocess.run(["pkill", "-9", "resolve"], stdin=subprocess.DEVNULL)
            else:
                subprocess.run(["pkill", "resolve"], stdin=subprocess.DEVNULL)
            return True
...
# Compatibility functions for migrated API
...
def quit_app(resolve, force: bool = False, save_project: bool = True) -> str:
    ...
    try:
        ...
        return "Quit request sent to DaVinci Resolve" if result else "Error: Failed to quit DaVinci Resolve"
    except Exception as e:
...
def open_settings(resolve) -> str:
    ...
    try:
        ...
        return "Opened project settings" if result else "Error: Failed to open project settings"
    except Exception as e:
        return f"Error: {e}"
...
def register_tools(proxy) -> int:
    """Register app control tools with the given proxy (ToolProxy)."""
    try:
    ...
    # Keep name compatible with src/api/app_operations.py
    if not _already_registered("quit_resolve"):
        proxy.register_tool(
            "quit_resolve",
            lambda force=False, save_project=True: quit_app(
                get_resolve() if get_resolve else None, force, save_project
            ),
            "app",
            "Quit DaVinci Resolve",
            {
    ...
    # Keep name compatible with resolve_mcp_server.py tool
    if not _already_registered("open_settings"):
        proxy.register_tool(
            "open_settings",
            lambda: open_settings(get_resolve() if get_resolve else None),
            "app",
