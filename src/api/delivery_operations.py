...
def add_to_render_queue(
    ...
    success_preset = False
    logger.info(f"Applying render preset: {preset_name}")

    # Try LoadRenderPreset first (standard method on project)
    try:
        if current_project.LoadRenderPreset(preset_name):
            logger.info(
                f"Successfully loaded preset using LoadRenderPreset: {preset_name}"
            )
            success_preset = True
        else:
            logger.warning(
                f"LoadRenderPreset returned False for '{preset_name}', trying fallback"
            )
    except Exception as e:
        logger.warning(f"LoadRenderPreset failed: {str(e)}, trying fallback")

    # Fallback to SetRenderSettings if LoadRenderPreset failed
    if not success_preset:
        settings_to_apply = {"SelectPreset": preset_name}
        if render_settings:
            settings_to_apply.update(render_settings)

        try:
            if render_settings_interface.SetRenderSettings(settings_to_apply):
                logger.info(f"Applied preset using SetRenderSettings: {preset_name}")
                success_preset = True
            else:
                logger.error(
                    f"Failed to apply render preset '{preset_name}' via SetRenderSettings"
                )
                return {"error": f"Failed to apply render preset '{preset_name}'"}
        except Exception as e:
            logger.error(f"Error applying render preset via fallback: {str(e)}")
            return {"error": f"Error applying render preset: {str(e)}"}
    elif render_settings:
        # If LoadRenderPreset succeeded but we also have custom settings
        try:
            if render_settings_interface.SetRenderSettings(render_settings):
                logger.info(f"Applied additional render settings: {render_settings}")
            else:
                logger.warning(
                    f"SetRenderSettings returned False for additional settings"
                )
        except Exception as e:
            logger.error(f"Error applying additional render settings: {str(e)}")

    # Add to render queue
    job_id = None
    logger.info("Adding to render queue")
    try:
        if use_in_out_range:
            logger.info("Using in/out range for render")
            # For in/out range, we need to ensure the range is set on the timeline
            # but usually AddRenderJob() uses the current timeline settings
            job_id = current_project.AddRenderJob()
        else:
            logger.info(f"Adding timeline '{actual_timeline_name}' to render queue")
            # AddTimelineToRenderQueue is one way, but AddRenderJob is often more robust
            # for the current timeline configuration
            job_id = current_project.AddRenderJob()
            if not job_id:
                logger.info("AddRenderJob failed, trying AddTimelineToRenderQueue")
                if current_project.AddTimelineToRenderQueue(actual_timeline_name):
                    # If it succeeded but didn't return an ID, try to get the last one
    ...
    if job_id:
        logger.info(f"Successfully added to render queue with Job ID: {job_id}")
        return {
            "success": True,
